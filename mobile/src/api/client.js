...
const fallbackUrl = 'http://10.72.2.50:5001/api';
export const API_URL = String(process.env.EXPO_PUBLIC_API_URL || fallbackUrl).replace(/\/$/, '');
...
export const unwrapItems = (data) => (Array.isArray(data) ? data : data?.items || data?.data || []);
export const unwrapList = (data) => {
  if (Array.isArray(data)) return { items: data, total: data.length, page: 1, totalPages: 1 };
  return { items: data?.items || data?.data || [], ...data };
};
