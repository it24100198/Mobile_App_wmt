...
export default function ExpenseList() {
  ...
  const openEdit = (e) => {
    ...
    setForm({
      ...
      date: formatDateInput(e.date) || emptyForm.date,
  ...
  const validateForm = () => {
    ...
    // Amount validation: required, number, >0, no negatives or zero, custom error
    if (
  ...
  const filtered = expenses.filter((expense) => {
    ...
    const dateStr = formatDateInput(expense.date);
                  ...
                  {paginated.map((e, index) => {
                    ...
                    return (
                      <tr key={e._id} className="group align-middle hover:bg-slate-50/70 transition-colors">
                        ...
                        <td className="px-4 py-3.5 text-slate-600 whitespace-nowrap">{formatDisplayDate(e.date)}</td>
